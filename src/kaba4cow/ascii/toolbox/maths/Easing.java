...
public enum Easing {
	...
	EASE_IN_OUT_BACK {
		...
		protected float calculate(float x) {
			...
			return 0.5f * (Maths.pow(2f * x - 2f, 2f) * ((c2 + 1f) * (x * 2f - 2f) + c2) + 2f);
		}
	},
	EASE_IN_ELASTIC {
		@Override
		protected float calculate(float x) {
			...
			if (x == 1f)
				return 1f;
			float c4 = Maths.TWO_PI * Maths.DIV3;
			return -Maths.pow(2f, 10f * x - 10f) * Maths.sin((10f * x - 10.75f) * c4);
		}
	},
	EASE_OUT_ELASTIC {
		@Override
		protected float calculate(float x) {
			...
			if (x == 1f)
				return 1f;
			float c4 = Maths.TWO_PI * Maths.DIV3;
			return Maths.pow(2f, -10f * x) * Maths.sin((10f * x - 0.75f) * c4) + 1f;
		}
	},
	EASE_IN_OUT_ELASTIC {
		@Override
		protected float calculate(float x) {
			...
			if (x == 1f)
				return 1f;
			float c5 = Maths.TWO_PI / 4.5f;
			if (x < 0.5f)
				return -0.5f * Maths.pow(2f, 20f * x - 10f) * Maths.sin((20f * x - 11.125f) * c5);
			return 0.5f * Maths.pow(2f, -20f * x + 10f) * Maths.sin((20f * x - 11.125f) * c5) + 1f;
		}
	},
	EASE_IN_BOUNCE {
		@Override
