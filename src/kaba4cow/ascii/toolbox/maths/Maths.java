...
public final class Maths {
	...
	public static float getBiasedFloat(float x, float start, float end, float bias) {
	...
	public static float map(float x, float start1, float stop1, float start2, float stop2) {
	...
	public static float mapBias(float x, float start1, float stop1, float start2, float stop2, float bias) {
		float value = map(x, start1, stop1, 0f, 1f);
		float biased = bias(value, bias);
		return map(biased, 0f, 1f, start2, stop2);
	}

	public static double mapBias(double x, double start1, double stop1, double start2, double stop2, double bias) {
		double value = map(x, start1, stop1, 0f, 1f);
		double biased = bias(value, bias);
		return map(biased, 0d, 1d, start2, stop2);
	}

	public static float mapLimit(float x, float start1, float stop1, float start2, float stop2) {
	...
	public static double mapLimit(double x, double start1, double stop1, double start2, double stop2) {
	...
	public static Vector2f blend(Vector2f left, Vector2f right, float blendFactor, Vector2f dest) {
		if (dest == null)
			dest = new Vector2f();
		float x = blend(left.x, right.x, blendFactor);
		float y = blend(left.y, right.y, blendFactor);
	...
	public static float manhattan(float x0, float y0, float z0, float x1, float y1, float z1) {
		return abs(x1 - x0) + abs(y1 - y0) + abs(z1 - z0);
	}
	...
	public static float dist(float x0, float y0, float z0, float w0, float x1, float y1, float z1, float w1) {
	...
	public static float distSq(float x0, float y0, float z0, float x1, float y1, float z1) {
		float distX = (x0 - x1) * (x0 - x1);
		float distY = (y0 - y1) * (y0 - y1);
	...
	public static float distSq(float x0, float y0, float z0, float w0, float x1, float y1, float z1, float w1) {
		float distX = (x0 - x1) * (x0 - x1);
		float distY = (y0 - y1) * (y0 - y1);
		float distZ = (z0 - z1) * (z0 - z1);
