package kaba4cow.ascii.toolbox.rng;
...
public abstract class RNG {
	...
	public float nextFloat(float min, float max) {
		if (max <= min)
			return min;
		return ((float) getNext() / (float) (0x7FFFFFFFFFFFFFFFl) * (max - min)) + min;
	}

	public double nextDouble(double min, double max) {
		if (max <= min)
			return min;
		return ((double) getNext() / (double) (0x7FFFFFFFFFFFFFFFl) * (max - min)) + min;
	}
