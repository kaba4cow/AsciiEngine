package kaba4cow.ascii.toolbox.noise;
...
import kaba4cow.ascii.toolbox.maths.vectors.Vector4f;
...
public class Noise implements Serializable {
	...
	public static Vector4f to01(Vector4f value) {
		return new Vector4f(to01(value.x), to01(value.y), to01(value.z), to01(value.w));
	}

	public static Vector4f to11(Vector4f vector) {
		return new Vector4f(to11(vector.x), to11(vector.y), to11(vector.z), to11(vector.w));
	}
	...
	public float getNoiseValue4(float d, Vector4f vector) {
		return getNoiseValue(d * vector.x, d * vector.y, d * vector.z, d * vector.w);
	}
	...
	public float getCombinedValue(float x, float y, float z, float w, int numOctaves) {
		float f = 1f;
		float a = 1f;
		float t = 0f;
		for (int i = 0; i < numOctaves; i++) {
	...
	public float getCombinedValue3(float d, Vector3f vector, int numOctaves) {
		return getCombinedValue(d * vector.x, d * vector.y, d * vector.z, numOctaves);
	}

	public float getCombinedValue4(float d, Vector4f vector, int numOctaves) {
		return getCombinedValue(d * vector.x, d * vector.y, d * vector.z, d * vector.w, numOctaves);
	}
	...
	public float getWarpedValue(float x, float y, float z, int sharpness, int complexity) {
		Vector3f fbmVec = new Vector3f(x, y, z);
		float fbm = 0;
		for (int i = 0; i < sharpness; i++) {
			fbm = to11(getCombinedValue(fbmVec.x, fbmVec.y, fbmVec.z, complexity));
			fbmVec.x += fbm;
			fbmVec.y += fbm;
			fbmVec.z += fbm;
		}
	...
	public float getWarpedValue(float x, float y, float z, float w, int sharpness, int complexity) {
		Vector4f fbmVec = new Vector4f(x, y, z, w);
		float fbm = 0;
		for (int i = 0; i < sharpness; i++) {
			fbm = to11(getCombinedValue(fbmVec.x, fbmVec.y, fbmVec.z, fbmVec.w, complexity));
			fbmVec.x += fbm;
			fbmVec.y += fbm;
			fbmVec.z += fbm;
			fbmVec.w += fbm;
		}
		return getCombinedValue(fbmVec.x, fbmVec.y, fbmVec.z, fbmVec.w, complexity);
	}

	public float getWarpedValue(float x, float y, int sharpness, int complexity, float power) {
		Vector2f fbmVec = new Vector2f(x, y);
		float fbm = 0;
		for (int i = 0; i < sharpness; i++) {
			fbm = to11(getCombinedValue(fbmVec.x, fbmVec.y, complexity));
	...
	public float getWarpedValue(float x, float y, float z, int sharpness, int complexity, float power) {
		Vector3f fbmVec = new Vector3f(x, y, z);
		float fbm = 0;
		for (int i = 0; i < sharpness; i++) {
			fbm = to11(getCombinedValue(fbmVec.x, fbmVec.y, fbmVec.z, complexity));
	...
	public float getWarpedValue(float x, float y, float z, float w, int sharpness, int complexity, float power) {
		Vector4f fbmVec = new Vector4f(x, y, z, w);
		float fbm = 0;
		for (int i = 0; i < sharpness; i++) {
			fbm = to11(getCombinedValue(fbmVec.x, fbmVec.y, fbmVec.z, fbmVec.w, complexity));
	...
	public float getWarpedValue2(float d, Vector2f vector, int sharpness, int complexity) {
		return getWarpedValue(d * vector.x, d * vector.y, sharpness, complexity);
	}

	public float getWarpedValue3(float d, Vector3f vector, int sharpness, int complexity) {
		return getWarpedValue(d * vector.x, d * vector.y, d * vector.z, sharpness, complexity);
	}

	public float getWarpedValue4(float d, Vector4f vector, int sharpness, int complexity) {
		return getWarpedValue(d * vector.x, d * vector.y, d * vector.z, d * vector.w, sharpness, complexity);
	}

	public float getWarpedValue2(float d, Vector2f vector, int sharpness, int complexity, float power) {
		return getWarpedValue(d * vector.x, d * vector.y, sharpness, complexity, power);
	}

	public float getWarpedValue3(float d, Vector3f vector, int sharpness, int complexity, float power) {
		return getWarpedValue(d * vector.x, d * vector.y, d * vector.z, sharpness, complexity, power);
	}

	public float getWarpedValue4(float d, Vector4f vector, int sharpness, int complexity, float power) {
		return getWarpedValue(d * vector.x, d * vector.y, d * vector.z, d * vector.w, sharpness, complexity, power);
	}

	public float getVoronoiValue(float x, float y, float manhattan, float randomness) {
	...
	public float getVoronoiValue(float x, float y, float z, float manhattan, float randomness) {
		float icx = Maths.floor(x);
		float icy = Maths.floor(y);
		...
		for (int px = -1; px <= 1; px++) {
			for (int py = -1; py <= 1; py++) {
				for (int pz = -1; pz <= 1; pz++) {
					...
					float dy = hash(153f + picx, 153f + picy, 153f + picz, randomness) - fcy;
					float dz = hash(87f + picx, 87f + picy, 87f + picz, randomness) - fcz;
	...
	private float hash(float x, float y, float z, float randomness) {
		...
		return Maths.blend(Maths.abs(x * y * z * (x + y + z)) % 1f, 0f, randomness);
	}
}